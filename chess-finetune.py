...
class JsonlDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        encoded = self.tokenizer(
            self.data[idx]["text"], return_tensors="pt", truncation=True, max_length=512
        )
...
def train(model, dataset, args):
    ...
    for step, batch in enumerate(dataloader):
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0 or step == len(
            ...
            logger.info(
                f"({training_step}/{num_training_steps}) loss={loss_training_step:.2f} ({time_elapsed:.2f}s/step)"
            )
            loss_training_step = 0
            lr_scheduler.step()
...
dataset = JsonlDataset(args.data_path, tokenizer)
